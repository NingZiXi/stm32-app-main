...
| §1 写入 | 拷贝 5 个 assets 模板到工程的 `main/` 目录 |
| §2 改根 CMake | 在根 `CMakeLists.txt` 加 `add_subdirectory(main)` |
| §3 接入口 | FreeRTOS → `freertos.c` USER CODE 5 / 裸机 → `main.c` USER CODE 2 |
| §4 业务搬迁 | 老工程 `Core/Src/` 里的业务文件由用户手动移到 `main/` |
| §5 验证 | `cmake -S/-B + cmake --build` |
...
| [assets/app_log.h](assets/app_log.h) | ~50 行 | 分级日志宏（`LOGI/LOGW/LOGE/LOGD` + ANSI 颜色） |
| [assets/app_log.c](assets/app_log.c) | ~20 行 | `_write` 重定向（newlib-nano 兼容） |
| [assets/app_main.c](assets/app_main.c) | 18 行 | FreeRTOS 版业务入口 |
| [assets/app_main_bare.c](assets/app_main_bare.c) | 17 行 | 裸机版业务入口 |
| [references/CMake-integration.md](references/CMake-integration.md) | — | `add_subdirectory` 方法论 |
...
```cmake
target_compile_definitions(${CMAKE_PROJECT_NAME} PRIVATE APP_LOG_COLORS=0)
```
...
| 现象 | 处理 |
|------|------|
| `add_subdirectory given source...` | `main/CMakeLists.txt` 没拷好，重跑 §1 |
...
| printf 输出全无（最常见） | `_write` 没生效，检查 `app_log.c` 是否被 `target_sources` 加入 |
